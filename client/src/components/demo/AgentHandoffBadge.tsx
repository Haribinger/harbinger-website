...
export default function AgentHandoffBadge({
  from,
  to,
  reason,
}: AgentHandoffBadgeProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-0 px-1 py-1 rounded-full border border-white/[0.06] bg-white/[0.02] overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-0.5">
          <div
            ...
            style={{
              ...
              border: `1px solid ${fromAgent.color}30`,
            }}
          >
            {fromAgent.icon}
          </div>
        ...
        {/* Animated arrow with glow trail */}
        <div className="relative w-16 h-5 mx-1 overflow-hidden">
          ...
          <svg
            ...
            height="12"
            viewBox="0 0 12 12"
            className="absolute right-0 top-1/2 -translate-y-1/2 text-[#00d4ff]"
          >
            <path
          ...
          {/* Animated packet with glow */}
          <motion.div
            className="absolute top-1/2 -translate-y-1/2 w-2 h-2 rounded-full"
            style={{
              background: `linear-gradient(90deg, ${fromAgent.color}, ${toAgent.color})`,
              boxShadow: `0 0 8px ${fromAgent.color}80`,
              ["--from-color" as string]: fromAgent.color,
              ["--to-color" as string]: toAgent.color,
            }}
            animate={{
              left: ["0%", "calc(100% - 8px)"],
              opacity: [0, 1, 1, 0],
              boxShadow: [
                `0 0 6px ${fromAgent.color}80`,
                `0 0 10px ${fromAgent.color}90`,
                `0 0 10px ${toAgent.color}90`,
                `0 0 6px ${toAgent.color}80`,
              ],
            }}
            transition={{
              duration: 1.5,
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-0.5">
          <div
            className="w-5 h-5 rounded-full flex items-center justify-center text-[10px]"
            style={{
              backgroundColor: toAgent.color + "20",
              border: `1px solid ${toAgent.color}30`,
            }}
          >
            {toAgent.icon}
          </div>
      ...
      <span className="text-[9px] text-[#555] max-w-xs text-center">
        {reason}
      </span>
    </motion.div>
  );
